...
func newTestClient(t *testing.T, handler http.Handler) *osrsClient {
	t.Helper()
	...
	cli := NewOSRSClient(logger).(*osrsClient)
	cli.client.SetRetryCount(0)
	cli.client.SetTransport(&rewriteToServerTransport{base: baseURL, next: http.DefaultTransport})
	return cli
}
